...
shinyUI(fluidPage(
  ...
  titlePanel("slimDashboard"),
  
  sidebarLayout(
    sidebarPanel(
      fileInput(
        'energy_file',
        'Choose energy data (.csv)',
        multiple = TRUE,
        accept = c('text/csv',
                 ...
                 tabsetPanel(
                   tabPanel(
                     ...
                     tabsetPanel(
                       tabPanel(
                         "Identification",
                         plotOutput("house_arima_acf", width = "95%", height = "720px") %>% withSpinner(color =
                                                                                                          "#000000")
                       ),
                       tabPanel(
                         "Estimation",
                         verbatimTextOutput("house_arima_estimate") %>% withSpinner(color = "#000000")
                       ),
                       tabPanel(
                         "Forecast",
                         dygraphOutput("house_arima", width = "95%", height = "720px") %>% withSpinner(color =
                                                                                                         "#000000")
                       ),
                       tabPanel(
                         "Diagnostics",
                         plotOutput("house_arima_tsdiag", width = "95%", height = "720px") %>% withSpinner(color =
                                                                                                             "#000000")
                       )
                     )
                   ),
                   tabPanel("Hidden Markov models",
                            tabsetPanel(
                              tabPanel(
                                "Estimation",
                                verbatimTextOutput("fitHMM") %>% withSpinner(color = "#000000")
                              ),
                              tabPanel(
                                "Inference",
                                plotOutput("house_hmm", width = "95%", height = "720px") %>% withSpinner(color =
                                                                                                           "#000000")
                              )
                            )),
                   tabPanel(
                     ...
                     tabsetPanel(
                       tabPanel(
                         "Estimation",
                         verbatimTextOutput("fitGAMM") %>% withSpinner(color = "#000000")
                       ),
                       tabPanel(
                         "Inference",
                         plotOutput("house_GAM", width = "95%", height = "720px") %>% withSpinner(color =
                                                                                                    "#000000")
                       )
                     )
                   )
